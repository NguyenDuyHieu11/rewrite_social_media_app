...
// Config holds only settings that running code consumes. Add fields when a
// feature ships, not before (MinIO, presence, etc. return with their code).
type Config struct {
	// ---- Shared ----
	AppEnv   Env
	LogLevel string
	...
	// ---- Gateway ----
	GatewayID   string // log correlation when running multiple gateways
	GatewayAddr string
